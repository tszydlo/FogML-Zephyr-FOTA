...
#ifdef CONFIG_FOGML_INTEGRATION
...
static const anjay_dm_object_def_t **ML_MODEL_OBJ;
static const anjay_dm_object_def_t **ANOMALY_ANALYZER_OBJ;
static const anjay_dm_object_def_t **ANOMALY_DETECTOR_OBJ;
#endif
...
#ifdef CONFIG_FOGML_INTEGRATION
	PATTERN_DETECTOR_OBJ = pattern_detector_object_create(FOGML_LABELS_COUNT, FOGML_LABELS);
	if (PATTERN_DETECTOR_OBJ) {
		anjay_register_object(anjay, PATTERN_DETECTOR_OBJ);
	}

    if ((ML_MODEL_OBJ = ml_model_object_create())) {
        anjay_register_object(anjay, ML_MODEL_OBJ);
    }

    if (FOGML_HAS_ANOMALY_DETECTION) {
        if ((ANOMALY_DETECTOR_OBJ = anomaly_detector_object_create())) {
            anjay_register_object(anjay, ANOMALY_DETECTOR_OBJ);
        }

#ifdef ANOMALY_ANALYZER
        if ((ANOMALY_ANALYZER_OBJ = anomaly_analyzer_object_create(FOGML_AXIS * FOGML_TIME_TICKS))) {
            anjay_register_object(anjay, ANOMALY_ANALYZER_OBJ);
        }
#endif
    }
#endif
...
static void release_objects(void)
{
    ...
    ml_model_object_release(ML_MODEL_OBJ);

    if (FOGML_HAS_ANOMALY_DETECTION) {
#ifdef ANOMALY_ANALYZER
        anomaly_analyzer_object_release(ANOMALY_ANALYZER_OBJ);
#endif
        anomaly_detector_object_release(ANOMALY_DETECTOR_OBJ);
    }
#endif
}
...
void main(void)
{
	...
	atomic_store(&fogml_verbose, false);
	process_sensor_data();
#endif
...
void update_anomaly_detector(float anomaly_score){
    LOG_INF("BEFORE ANOMALY DETECTOR UPDATE");
    anomaly_detector_object_update(global_anjay, ANOMALY_DETECTOR_OBJ, anomaly_score);
    LOG_INF("AFTER ANOMALY DETECTOR UPDATE");
}

void update_pattern_detector(int detected_class){
    LOG_INF("BEFORE PATTERN DETECTOR UPDATE");
    pattern_detector_object_update_detected_class(global_anjay, PATTERN_DETECTOR_OBJ, detected_class);
    LOG_INF("AFTER PATTERN DETECTOR UPDATE");
}

void update_anomaly_analyzer(float anomaly_score, float* sensor_data){
    LOG_INF("BEFORE ANOMALY ANALYZER UPDATE");
    anomaly_analyzer_object_update_sensor_data(global_anjay, ANOMALY_ANALYZER_OBJ, anomaly_score, sensor_data);
    LOG_INF("AFTER ANOMALY ANALYZER UPDATE");
}
#endif